...
int main(){
	...
	if(input_file.is_open() && output_file.is_open()){
		while(true){
			...
			input_file >> qid >> purc_k_id >> titid >> desc_id >> index >> click >> impression >> display_url >> ad_id >> advertiser_id >> depth >> position >> user_id >> desc_id_tokencnt >> titid_tokencnt >> purc_k_id_tokencnt >> qid_tokencnt;	
