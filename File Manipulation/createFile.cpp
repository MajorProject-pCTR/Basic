...
int main(){
	...
	if(input_file.is_open() && output_file.is_open()){
		while(true){
			...
			for(j=1; j<=click; ++j){
				output_file << 1 << " " << ad_id << " " << advertiser_id << " " << depth << " " << position << " " << query_id << " " << keyword_id << " " << title_id << " " << description_id << " " << user_id << "\n";
				index++;
			}
	
			for(j=1; j<=impression-click; ++j){
				output_file << 0 << " " << ad_id << " " << advertiser_id << " " << depth << " " << position << " " << query_id << " " << keyword_id << " " << title_id << " " << description_id << " " << user_id << "\n";
				index++;
			}
